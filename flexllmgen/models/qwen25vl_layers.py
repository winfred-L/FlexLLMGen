...
class Qwen2_5_VLAttention(SelfAttention):
    ...
    def load_cache_sparse(self, cache_home, cache_read_buf, i, attention_mask, prev_hidden):
        '''
        load sparse kv cache to GPU.
        when prev_hidden is None, only static sparsity is used.
        only cache_home split text and video kv, cache_read_buf and cache_write_buf does not.
        '''
        assert self.attention_compute == self.env.gpu
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if i == 0:  # prefill
            ...
            if self.policy.do_sparse: # sparse attention
                ...
                h, new_k_cache, new_v_cache, _ = self.compute.qwen25vl_gqa_gen_sparse(h, mask, w_q,
                ...
                mask, donate[1] = attention_mask.val.smart_copy(self.attention_compute)
                (k_cache, donate[10]), (v_cache, donate[11]) = cache_read_buf.pop()
                h, new_k_cache, new_v_cache, attn_weight = self.compute.qwen25vl_gqa_gen(h, mask, w_q,
                    b_q, w_k, b_k, w_v, b_v, w_out, w_ln, n_head, n_kv_head,
                    k_cache, v_cache, donate, self.policy.attn_sparsity,
                    self.policy.compress_cache, self.policy.comp_cache_config,
                    self.config.rms_norm_eps, position_embeddings, self.config.rope_scaling_mrope_section,
                ...
                # analyse attn weight
                if self.policy.attn_impl == 'eager':
