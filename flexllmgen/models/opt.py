...
from flexllmgen.models.layers import InputEmbed, OutputEmbed, SelfAttention, MLP, TransformerLayer

class OptLM:
    '''
    OptLM 类：管理 OPT 模型的执行流程，核心功能包括：
    1. 内存管理：初始化权重、KV Cache 和隐藏层状态的缓冲区。
    2. 设备调度：根据策略 (Policy) 将数据在 Disk/CPU/GPU 之间移动。
    3. 流水线执行：控制生成过程中的预填充 (Prefill) 和解码 (Decoding) 阶段。
    '''
    def __init__(self,
        ...
        # --- 初始化网络层 ---
        # 按顺序构建模型：输入Embedding -> Transformer层 * layer_num -> 输出Embedding
        layers = []
