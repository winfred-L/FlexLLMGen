...
def add_parser_arguments(parser):
    ...
    parser.add_argument("--pin-weight", type=str2bool, nargs="?", # 是否将权重锁页到内存以加快 CPU 访问速度
        const=True, default=True)
    parser.add_argument("--cpu-cache-compute", action="store_true") # 是否在 CPU 上计算 Attention
    parser.add_argument("--attn-sparsity", type=float, default=1.0) # Attention 的稀疏度，1.0 代表稠密
    parser.add_argument("--compress-weight", action="store_false",
        help="Whether to compress weight.")
    parser.add_argument("--compress-cache", action="store_false",
        help="Whether to compress cache.")


    parser.add_argument("--log-file", type=str, default="auto") # 日志文件名
    parser.add_argument("--no-log", action="store_true") # 不记录日志
